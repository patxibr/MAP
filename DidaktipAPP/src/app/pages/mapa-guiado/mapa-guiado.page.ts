...
@Component({
  selector: 'app-mapa-guiado',
  templateUrl: './mapa-guiado.page.html',
  styleUrls: ['./mapa-guiado.page.css'],
})
export class MapaGuiadoPage implements OnInit {
  ...
  leafletMap(route:Router) {
    ...
    .then(data => {
      this.txt = data;
      this.restProvider.getLastCompleted().then(data2 =>{
        for (const property of this.txt) {
          ...
          }else if(property.order < data2['completion']){
            this.markerIcon = Leaflet.icon(
              {
                iconUrl: '../../../assets/icon/icoMarkerComplete.png',
                iconSize: [20, 31],
              }
            );
          } else {
          ...
          if(property.order <= data2['completion']){
